...
public class SelectionHelper
{
    ...
    static void selectByType(MenuCommand selected,bool child)
    {
        ...
        Component[] comps;
        if (child)
            comps = ((GameObject)selected.context).GetComponentsInChildren(myCurrentType, true);
        else
            comps = ((GameObject)selected.context).GetComponentsInParent(myCurrentType, true);
